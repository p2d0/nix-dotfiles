...
function make-youtube-clip -d "Download trim and copy to clipboard youtube video"
    ...
    if set -q _flag_realsubs
        ...
        yt-dlp --write-sub --sub-lang $_flag_realsubs  --no-continue $argv[1] -o tmp_vid.mp4 -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best" --merge-output-format mp4
        mv "tmp_vid.$_flag_realsubs.vtt" tmp_vid.vtt
    else if set -q _flag_subs
        echo "Auto generated subs"
        yt-dlp --write-auto-sub --no-continue $argv[1] -o tmp_vid.mp4 -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best" --merge-output-format mp4
        clean_youtube_dl_auto_subs tmp_vid.en.vtt > tmp_vid.vtt;
    else
        yt-dlp --no-continue $argv[1] -o tmp_vid.mp4 -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best" --merge-output-format mp4
    end
